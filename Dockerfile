...
# Set up directories
RUN mkdir -p /workspace /home/dev/.claude /home/dev/.ssh && \
...
# Fix ownership
RUN chown -R dev:dev /home/dev /usr/local/share/npm-global

# Move uv tools to dev user
RUN cp -r /root/.local/bin/* /usr/local/bin/ 2>/dev/null || true && \
    cp -r /root/.local/share/uv /home/dev/.local/share/uv 2>/dev/null || true && \
